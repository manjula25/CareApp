...
7. **Post-Commit-4 regression discovered during live smoke test (FIXED in `f8d0862`).** The live smoke test of the analysis endpoint hit a regression that 281/281 unit tests couldn't catch: smartAuth was mounted via `app.use('/api/patients', smartAuth, createPatientsRouter(fhirService))` which ran smartAuth BEFORE the route's inner `requireAuth`. Every legitimate login-JWT caller was rejected with `{"error":"smart_auth_failed","reason":"invalid_signature"}` because smartAuth verifies against the SMART `serverSecret` and login JWTs are signed with `JWT_SECRET`. Fixed in `f8d0862`: (a) added a 4-line `if (req.auth) return next()` pass-through at the top of smartAuth; (b) moved the smartAuth mount INSIDE each route via a new `wrapRouterWithSmartAuth(router, smartAuth)` helper. New 6th unit test pins the pass-through. Live smoke test (after fix): login JWT → 200, no token → 401 from requireAuth, garbage token → 401 from requireAuth. **Methodology lesson**: mount-order bugs don't surface from per-route test apps (each test file builds its own Express app without the global mount chain). The S14 unit-test strategy is insufficient for this class of bug; future slices need at least one integration smoke test against the running `npm run dev` server.

8. **requireAuth should learn SMART-shape tokens too.** Following from follow-up #7's fix: smartAuth now passes through login JWTs (because requireAuth already validated them). But requireAuth itself still rejects SMART-shape tokens — so a SMART-token-only caller (not a current POC use case, but conceivable for a future backend-to-backend integration) would 401 at requireAuth before smartAuth ever runs. The clean fix is to teach requireAuth to accept either token shape; out of scope for `f8d0862` (the immediate regression was just the mount order).
